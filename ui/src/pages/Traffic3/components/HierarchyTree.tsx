...
const TreeCard = styled(Card)`
  ...
  .ant-tree .ant-tree-node-content-wrapper.ant-tree-node-selected {
    background: linear-gradient(90deg, rgba(139, 92, 246, 0.12), rgba(139, 92, 246, 0.06)) !important;
    border-left: 2px solid rgba(139, 92, 246, 0.6);
    padding-left: 4px;

    [data-theme="light"] & {
      background: linear-gradient(90deg, rgba(139, 92, 246, 0.1), rgba(139, 92, 246, 0.04)) !important;
...
const CardTitle = styled.h3`
  ...
  transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1);
  letter-spacing: -0.01em;
  background: linear-gradient(135deg, var(--color-text-heading), var(--color-text-base));
...
function buildBindTitle(
  ...
  return (
    ...
    >
      <ResourceIcon icon={<Network />} color={getResourceColor("bind")} size="small" />
...
function buildListenerTitle(
  ...
  return (
    ...
    >
      <ResourceIcon icon={<Headphones />} color={getResourceColor("listener")} size="small" />
...
function buildPolicyTitle(
  ...
  return (
    ...
    >
      <ResourceIcon icon={<Settings />} color={getResourceColor("policy")} size="small" />
...
function buildBackendTitle(
  ...
  return (
    ...
    >
      <ResourceIcon icon={<Server />} color={getResourceColor("backend")} size="small" />
...
function buildModelTitle(
  ...
  return (
    ...
    >
      <ResourceIcon icon={<Bot />} color={getResourceColor("model")} size="small" />
...
function buildTreeData(
  ...
  if (hierarchy.frontendPolicies) {
    nodes.push({
      ...
      title: buildTopLevelItemTitle(
        "Frontend Policies",
        <ResourceIcon icon={<Settings />} color={getResourceColor("frontendPolicies")} />,
        true,
        ...
        .map((listener, li) => ({
          ...
          children: listener.routes
            ...
            .map((route) => {
              ...
              const sortedBackends = route.backends
                .slice()
                .sort((a, b) => {
  ...
  const handleAddRoute = useCallback(async (port: number, li: number, isTcp: boolean) => {
    try {
      ...
      if (isTcp) {
        ...
        updatedListener.tcpRoutes = listener.tcpRoutes ? [...listener.tcpRoutes, newRoute] : [newRoute];
        // Remove routes field if it exists
        delete updatedListener.routes;
      } else {
        // Only include routes
        updatedListener.routes = listener.routes ? [...listener.routes, newRoute] : [newRoute];
